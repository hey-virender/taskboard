...
const TaskComponent = ({task, onDelete}:{task:Task,onDelete:({task_id}:{task_id:number})=>void}) => {
  ...
  return (
    <div ref={ref} style={{ opacity: isDragging ? 0.5 : 1 }}  id={task.id.toString()} className={`h-32 w-full p-3 relative border-1 rounded-lg ${task.status == "todo" && "border-blue-500"} ${task.status == "in_progress" && "border-yellow-500"} ${task.status == "done" && "border-green-500"}`}>
